...
export default function Page() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const data = await authRequest(apiEndpoints.categorias, { token })
        setItems(asArray(data) as Categoria[])
      } catch (err) {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      setSaving(true)
      if (editingId) {
        const updated = (await authRequest(`${apiEndpoints.categorias}${editingId}/`, {
          method: "PUT",
          body: payload,
          token,
        })) as Categoria
        ...
        const created = (await authRequest(apiEndpoints.categorias, {
          method: "POST",
          body: payload,
          token,
        })) as Categoria
  ...
  const onDelete = async (item: Categoria) => {
    ...
    try {
      await authRequest(`${apiEndpoints.categorias}${item.id}/`, { method: "DELETE", token })
