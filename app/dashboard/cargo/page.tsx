...
export default function Page() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const data = await authRequest(apiEndpoints.cargos, { token })
        setItems(asArray(data) as Cargo[])
      } catch (err) {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      setSaving(true)
      if (editingId) {
        const updated = (await authRequest(`${apiEndpoints.cargos}${editingId}/`, {
          method: "PUT",
          body: payload,
          token,
        })) as Cargo
        ...
        const created = (await authRequest(apiEndpoints.cargos, {
          method: "POST",
          body: payload,
          token,
        })) as Cargo
  ...
  const onDelete = async (item: Cargo) => {
    ...
    try {
      await authRequest(`${apiEndpoints.cargos}${item.id}/`, { method: "DELETE", token })
